...
- Product Management and QA are available Godemy directions. Keep their courses at `/product` and `/qa`; their course-specific practice routes are `/product/practice` and `/qa/practice`. Each programme has six modules and a Bit Tech work-simulation context.
- From the universal landing, course CTAs open the selected course cabinet directly. Every course cabinet uses the same Yandex-Practicum-inspired structure: compact left rail, progress card, course-specific practice action, calm study-rhythm prompt, role note, and a six-card module grid; do not insert an intermediate mini-landing.
- Module cards use a two-part editorial anatomy inspired by the supplied Practicum reference: a generous white infographic area above the title/meta/progress area. Give all six modules distinct thin-line compositions, and use different semantic icon sets and accents for Go, SQL, and Python rather than repeating one generic illustration.
...
- In every course cabinet, show practice as a dedicated block immediately below the top course progress card rather than as a secondary action inside it. Do not show the generic weekly study-rhythm prompt.
- Certificates belong only to complete course tracks, never to practice catalogues or standalone projects. The certificate catalogue is Go Junior Developer, Python Junior Developer, SQL Junior Developer, Agile, and Scrum; show unreleased courses as future programmes rather than progress already in motion.
