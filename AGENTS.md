...
- Keep the public product architecture two-level: `/` is the focused Go marketing landing, while `/academy` is the compact learning hub with two primary choices — the full course and the Go trainer. Public CTAs lead to `/academy` before entering the learning surfaces.
